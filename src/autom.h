...
void Automatic::Letter(char letter) {
...
void Automatic::AddTrans(const int from, const char letter, const int to) {
...
bool Automatic::Check (std::string word) {
    if (!flag) {
        std::cout << "Bad structure\n";
        return false;
    }
    if(word == "$") {
        if (graph[start].final == true) {
            return true;
        }
    }
